...
void init_style_dark_info_bar_scroll_text_MAIN_DEFAULT(lv_style_t *style) {
    lv_style_set_text_color(style, lv_color_hex(0xffff6b6b));
    lv_style_set_text_font(style, &lv_font_montserrat_10);
    lv_style_set_text_opa(style, 255);
    lv_style_set_text_align(style, LV_TEXT_ALIGN_LEFT);
...
lv_style_t *get_style_dark_info_bar_scroll_text_MAIN_DEFAULT() {
    ...
    if (!style) {
        style = (lv_style_t *)lv_malloc(sizeof(lv_style_t));
        lv_style_init(style);
        init_style_dark_info_bar_scroll_text_MAIN_DEFAULT(style);
    }
    return style;
};
...
void add_style_dark_info_bar_scroll_text(lv_obj_t *obj) {
    (void)obj;
    lv_obj_add_style(obj, get_style_dark_info_bar_scroll_text_MAIN_DEFAULT(), LV_PART_MAIN | LV_STATE_DEFAULT);
};

void remove_style_dark_info_bar_scroll_text(lv_obj_t *obj) {
    (void)obj;
    lv_obj_remove_style(obj, get_style_dark_info_bar_scroll_text_MAIN_DEFAULT(), LV_PART_MAIN | LV_STATE_DEFAULT);
};
